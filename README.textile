...
you can set your prefer segment mode,default `use_smart` and `merge_single_char` are false.
...
curl -XPOST http://localhost:9200/test/fulltext/1 -d'
{content:"美国留给伊拉克的是个烂摊子吗"}
'

curl -XPOST http://localhost:9200/test/fulltext/2 -d'
{content:"公安部：各地校车将享最高路权"}
'

curl -XPOST http://localhost:9200/test/fulltext/3 -d'
{content:"中韩渔警冲突调查：韩警平均每天扣1艘中国渔船"}
'

curl -XPOST http://localhost:9200/test/fulltext/4 -d'
...
curl -XGET http://localhost:9200/test/fulltext/_search  -d'
{
